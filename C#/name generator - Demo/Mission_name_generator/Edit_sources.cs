...
namespace Mission_name_generator
{
    public partial class Edit_sources : Form
    {
        ...
        private void Edit_sources_Load(object sender, EventArgs e)
        {
            ...
            Mission_prefix_list.Items.Clear();
            Mission_suffix_list.Items.Clear();
            ...
            //Sorts the lists
            Mission_prefix_list.Sorted = true;
            Mission_suffix_list.Sorted = true;
            ...
            string [] Mission_prefixes = System.IO.File.ReadAllLines(@"..\..\..\Generator components\Mission Prefix list.txt");
            string [] Mission_suffixes = System.IO.File.ReadAllLines(@"..\..\..\Generator components\Mission suffix list.txt");
            ...
            int Record_number = 0;
            while (Record_number < Mission_prefixes.Length)
            {
                Mission_prefix_list.Items.Add(Mission_prefixes[Record_number].ToString());
                Record_number++;
            }
            Record_number = 0;
            while (Record_number < Mission_suffixes.Length)
            {
                Mission_suffix_list.Items.Add(Mission_suffixes[Record_number].ToString());
                Record_number++;
            }
            Record_number = 0;
        ...
        private void Add_prefix_BTN_Click(object sender, EventArgs e)
        {
            //Checks if the textbox is empty and adds the contents of the textbox if not.
            if (string.IsNullOrWhiteSpace(Prefix_box.Text))
            {
                MessageBox.Show("please enter a value first");
            }
            else
                Mission_prefix_list.Items.Add(Prefix_box.Text);
            Prefix_box.Text = string.Empty;
        }
        //These are the other 5 add buttons, colapsed for compactness, they are almost identical to the add prefix option
        private void Add_suffix_BTN_Click(object sender, EventArgs e)
        {
            if(string.IsNullOrWhiteSpace(suffix_box.Text))
            {
                MessageBox.Show("please enter a value first");
            }
            else
                Mission_suffix_list.Items.Add(suffix_box.Text);
            suffix_box.Text = string.Empty;
        }
        ...
        private void Delete_prefix_BTN_Click(object sender, EventArgs e)
        {
            //Removes the entry in the selected position from the list
            Mission_prefix_list.Items.Remove(Mission_prefix_list.SelectedItem);
        }
        //These are the other 5 delete buttons, colapsed for compactness, they are almost identical to the delete prefix option
        private void Delete_Suffix_BTN_Click(object sender, EventArgs e)
        {
            //Removes the entry in the selected position from the list
            Mission_suffix_list.Items.Remove(Mission_suffix_list.SelectedItem);
        }
        ...
        //Edit selected entry buttons
        private void Edit_prefix_BTN_Click(object sender, EventArgs e)
        {
            //Maks sure the user has both filled in the box and selected the item, doing this without checking would crash the program
            if (string.IsNullOrWhiteSpace(Prefix_box.Text))
            {
                MessageBox.Show("Please make sure you have entered the edited word");
                return;
            }
            else if (Mission_prefix_list.SelectedItem == null)
            {
            ...
            else
            {
                ...
                int selected = Mission_prefix_list.Items.IndexOf(Mission_prefix_list.SelectedItem);
                Mission_prefix_list.Items.RemoveAt(selected);
                Mission_prefix_list.Items.Insert(selected, Prefix_box.Text);
                Prefix_box.Text = string.Empty;
            }
            
        }
        //These are the other 5 edit buttons, colapsed for compactness, they are almost identical to the edit prefix option
        private void Edit_suffix_BTN_Click(object sender, EventArgs e)
        {
            ...
            else if (Mission_suffix_list.SelectedItem == null)
            {
                MessageBox.Show("Please select the word to be replaced");
                return;
            }
            ...
            {
                //Gets the index of the item, removes the existing item and enters the new word in it's place
                int selected = Mission_suffix_list.Items.IndexOf(Mission_suffix_list.SelectedItem);
                Mission_suffix_list.Items.RemoveAt(selected);
                Mission_suffix_list.Items.Insert(selected, suffix_box.Text);
                suffix_box.Text = string.Empty;
            }
        }
        ...
        private void Save_changes_btn_Click(object sender, EventArgs e)
        {
            ...
            if (List_to_save_Select.Text == "Mission prefixes")
            {
                //Sorts list alphabeticaly, converts the list to the update array and defines the file location
                Mission_prefix_list.Sorted = true;
                Updated_list = Mission_prefix_list.Items.OfType<string>().ToArray();
                File_location = @"..\..\..\Generator components\Mission Prefix list.txt";
            }                                 
            
            else if (List_to_save_Select.Text == "Mission suffixes ")
            {
                //Sorts list alphabeticaly, converts the list to the update array and defines the file location
                Mission_suffix_list.Sorted = true;
                Updated_list = Mission_suffix_list.Items.OfType<string>().ToArray();
                File_location = @"..\..\..\Generator components\Mission suffix list.txt";
            }

            else if (List_to_save_Select.Text == "Male first names")
            {
                //Sorts list alphabeticaly, converts the list to the update array and defines the file location
                Male_first_name_list.Sorted = true;
        ...
        {
            Mission_prefix_list.Sorted = false;
            Mission_suffix_list.Sorted = false;
        ...
        private void Deselect_BTN_Click(object sender, EventArgs e)
        {
            ...
            Mission_prefix_list.ClearSelected();
            Mission_suffix_list.ClearSelected();
